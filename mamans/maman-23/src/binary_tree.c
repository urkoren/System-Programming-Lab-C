...
void fparse_to_findex(FILE *file, struct index *findex) {
  int line_count = 1;
  char line[MAXWORD];
  char *key = NULL;
...
struct tnode *addtree(struct tnode *p, char *w, int line_count) {
  int cond;
...
struct line_node *create_new_line_node(int line_count) {
...
int check_line(struct tnode *p, int line_count) {
...
void append_line_count(struct tnode *p, int line_count) {
  struct line_node *new_node;
  new_node = lalloc();
  if (new_node == NULL) {
    printf("Memory allocation failed\n");
...
void reset_findex(struct index *findex) {
  if (findex->root != NULL) {
...
void free_line_list(struct line_node *lines) {
